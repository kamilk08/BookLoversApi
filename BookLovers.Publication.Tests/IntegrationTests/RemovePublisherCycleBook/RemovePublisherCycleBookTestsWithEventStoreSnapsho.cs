...
{
    public class RemovePublisherCycleBookTestsWithEventStoreSnapshot :
        IntegrationTest<PublicationModule, RemovePublisherCycleBookCommand>
    {
        ...
        public async Task RemovePublisherCycleBook_WhenCalled_ShouldRemoveBookAndReturnValidationResultWithoutErrors()
        {
            ...
            var cycle = await this.Module.ExecuteQueryAsync<CycleByGuidQuery, PublisherCycleDto>(
                new CycleByGuidQuery(_publisherCycleGuid));

            cycle.Value.CycleBooks.Should().HaveCount(0);
        }
        ...
        protected override void InitializeRoot()
        {
            ...
            PublicationModuleStartup.Initialize(
                new FakeHttpContextAccessor(this._readerGuid, true), appManagerMock.Object,
        ...
        protected override async Task PrepareData()
        {
            ...
            {
                await Module.SendCommandAsync(new AddPublisherCycleBookCommand(cycle.Guid, book.Guid));
                await Module.SendCommandAsync(new RemovePublisherCycleBookCommand(book.Guid, cycle.Guid));
            });

            await this.Module.SendCommandAsync(new AddPublisherCycleBookCommand(cycle.Guid, book.Guid));
