...
{
    public class RemovePublisherCycleAndPublisherCycleDoesNotExist : EndToEndTest
    {
        ...
        [Test]
        public async Task
            RemoveBookFromCycle_WhenCalledAndCycleDoesNotExist_ShouldReturnResponseWithConflictStatusCode()
        {
            await this.E2ETester
                .ArePrecedingRequestsSuccessful((flag) => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(() =>
                    new RemoveBookFromPublisherCycleHttpRequest(_bookGuid, Fixture.Create<Guid>(), _token))
