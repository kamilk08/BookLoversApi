...
namespace BookLovers.Publication.Tests.EndToEndTests.ArchivePublisherCycleTest
{
    public class ArchivePublisherCycleTest : EndToEndTest
    {
        ...
        [Test]
        public async Task ArchivePublisherCycle_WhenCalled_ShouldReturnResponseWithOkStatusCode()
        {
            ...
            await this.E2ETester
                ...
                .SendAsyncAndMapToResult(() => new ArchivePublisherCycleHttpRequest(_publisherCycleGuid, _token))
                .Tap((result) => result.IsOk().Should().BeTrue())
                .Back()
                .AddTask(async () => await poller.Check(new ArchivePublisherCycleRatingsModuleProbe(ratingsModule)))
                .DoAsync();
        }
        ...
        protected override async Task SendPreRequestsAsync()
        {
            await this.E2ETester
                ...
                .Act(() => _publisherCycleGuid = Fixture.Create<Guid>())
                ...
                .SendSynchronouslyAndMapToResult(LoginAdmin)
                .Tap(async (result) => this._token = await result.GetResponseContentAsync<JsonWebToken>())
                .Back()
                .SendAsync(() => new CreateLibrarianHttpRequest(_token)
                    .WithReaderGuid(_userGuid))
        ...
        protected override void ConfigureLogger()
        {
            Logger = new LoggerConfiguration().Enrich
                .WithExceptionDetails().MinimumLevel
                .Debug().WriteTo
                .Console(
        ...
        private LoginUserRequest LoginAdmin()
        {
            return new LoginUserRequest()
