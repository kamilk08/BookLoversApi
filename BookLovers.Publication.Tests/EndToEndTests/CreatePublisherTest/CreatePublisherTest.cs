...
{
    public class CreatePublisherTest : EndToEndTest
    {
        ...
        [Test]
        public async Task CreatePublisher_WhenCalled_ShouldReturnResponseWithCreatedStatusCode()
        {
            ...
            await this.E2ETester
                ...
                .SendAsyncAndMapToResult(() =>
                    (ISimpleHttpRequest) new CreatePublisherHttpRequest(_token).WithPublisherGuid(_publisherGuid))
                .Tap(result => result.IsCreated().Should().BeTrue()).Back().AddTaskAsync(async () =>
                    await poller.Check(new CreatePublisherRatingsModuleProbe(module, _publisherGuid))).DoAsync();
        }
