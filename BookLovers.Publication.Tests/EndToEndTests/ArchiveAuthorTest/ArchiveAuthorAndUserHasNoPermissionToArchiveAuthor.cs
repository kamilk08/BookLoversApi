...
namespace BookLovers.Publication.Tests.EndToEndTests.ArchiveAuthorTest
{
    public class ArchiveAuthorAndUserHasNoPermissionToArchiveAuthor : EndToEndTest
    {
        ...
        [Test]
        public async Task
            ArchiveAuthor_WhenCalledAndUserHasNoPermissionToDoIt_ShouldReturnResponseWithForbiddenStatusCode()
        {
            await this.E2ETester
                .ArePrecedingRequestsSuccessful((flag) => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(() => new ArchiveAuthorHttpRequest(_authorGuid, _token))
        ...
        protected override async Task SendPreRequestsAsync()
        {
            await this.E2ETester
                ...
                .Tap(async (result) => this._token = await result.GetResponseContentAsync<JsonWebToken>())
                .Back()
                .SendAsync(this.CreateAuthor)
                .ActAsync(async () => await Task.Run(() =>
                {
                    var fakeHttpContextAccessor =
                        new FakeHttpContextAccessor(Fixture.Create<Guid>(), true, false, false);
