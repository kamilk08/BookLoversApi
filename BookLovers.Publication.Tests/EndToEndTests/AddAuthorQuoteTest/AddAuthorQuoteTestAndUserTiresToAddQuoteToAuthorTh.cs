...
namespace BookLovers.Publication.Tests.EndToEndTests.AddAuthorQuoteTest
{
    public class AddAuthorQuoteTestAndUserTiresToAddQuoteToAuthorThatDoesNotExist : EndToEndTest
    {
        ...
        [Test]
        public async Task CreateAuthorQuote_WhenCalledAndAuthorDoesNotExist_ShouldReturnResponseWithConflictStatusCode()
        {
            await this.E2ETester
                .ArePrecedingRequestsSuccessful((flag) => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(() => new CreateAuthorQuoteHttpRequest(Fixture.Create<Guid>(), _token))
