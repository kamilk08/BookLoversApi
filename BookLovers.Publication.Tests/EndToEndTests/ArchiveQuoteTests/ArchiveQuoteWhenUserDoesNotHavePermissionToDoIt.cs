...
namespace BookLovers.Publication.Tests.EndToEndTests.ArchiveQuoteTests
{
    public class ArchiveQuoteWhenUserDoesNotHavePermissionToDoIt : EndToEndTest
    {
        ...
        [Test]
        public async Task
            ArchiveQuote_WhenCalledAndUserDoesNotHavePermissionToDoIt_ResponseShouldHaveForbiddenStatusCode()
        {
            await this.E2ETester
                .ArePrecedingRequestsSuccessful((flag) => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(() => new ArchiveQuoteHttpRequest(_quoteGuid, _token))
        ...
        protected override async Task SendPreRequestsAsync()
        {
            await this.E2ETester
                ...
                .SendAsync(() =>
                    new CreateAuthorQuoteHttpRequest(_authorGuid, _token).WithQuoteGuid(_quoteGuid))
                .ActAsync(async () => await Task.Run(() =>
                {
                    var fakeHttpContextAccessor = new FakeHttpContextAccessor(_secondUserGuid, true, false, false);
                    this.Kernel.Rebind<IHttpContextAccessor>().ToConstant(fakeHttpContextAccessor);
                }));
        }
