...
namespace BookLovers.Publication.Tests.EndToEndTests.UnLikeAuthorQuoteTest
{
    public class UnLikeAuthorQuoteAndQuoteDoesNotExist : EndToEndTest
    {
        ...
        [Test]
        public async Task UnLikeAuthorQuote_WhenCalledAndQuoteDoesNotExist_ShouldReturnResponseWithConflictStatusCode()
        {
            await E2ETester
                .ArePrecedingRequestsSuccessful(flag => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(new UnLikeAuthorQuoteHttpRequest(Fixture.Create<Guid>(), _token))
        ...
        protected override async Task SendPreRequestsAsync()
        {
            await E2ETester
                ...
                .Act(() => this._quoteGuid = Fixture.Create<Guid>())
                ...
                .SendAsync(new CreateAuthorQuoteHttpRequest(_authorGuid, _token).WithQuoteGuid(_quoteGuid))
                .SendAsync(new LikeAuthorQuoteHttpRequest(_quoteGuid, _token));
        }

        protected override void ConfigureLogger()
        {
            Logger = new LoggerConfiguration()
                .Enrich.WithExceptionDetails().MinimumLevel.Debug().WriteTo
                .Console(
