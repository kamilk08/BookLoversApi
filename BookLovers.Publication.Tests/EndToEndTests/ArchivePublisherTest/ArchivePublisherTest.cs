...
namespace BookLovers.Publication.Tests.EndToEndTests.ArchivePublisherTest
{
    public class ArchivePublisherTest : EndToEndTest
    {
        ...
        [Test]
        public async Task ArchivePublisher_WhenCalled_ShouldReturnResponseWithOkStatusCode()
        {
            IModule<RatingsModule> module = new RatingsModule();
            ...
            await this.E2ETester
                ...
                .SendAsyncAndMapToResult(() => new ArchivePublisherHttpRequest(_publisherGuid, _token))
                .Tap((result) => result.IsOk().Should().BeTrue())
                .Back()
                .AddTask(async () => await poller.Check(new ArchivePublisherInRatingsModuleProbe(module)))
                .DoAsync();
        }
        ...
        protected override async Task SendPreRequestsAsync()
        {
            await this.E2ETester
                ...
                .Tap(async (result) => this._token = await result.GetResponseContentAsync<JsonWebToken>())
                .Back()
                .SendAsync(() => new CreatePublisherHttpRequest(_token).WithName(SelfPublisher.Key))
                .SendAsync(CreateNewPublisher);
        }
        ...
        private CreatePublisherHttpRequest CreateNewPublisher()
        {
