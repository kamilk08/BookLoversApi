...
{
    public class AddBookToPublisherCycleTestAndBookDoesNotExist : EndToEndTest
    {
        ...
        [Test]
        public async Task
            AddBookToPublisherCycle_WhenCalledAndBookDoesNotExist_ShouldReturnResponseWithConflictStatusCode()
        {
            await this.E2ETester
                .ArePrecedingRequestsSuccessful((flag) => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(() =>
                    new AddToPublisherCycleHttpRequest(Fixture.Create<Guid>(), _publisherCycleGuid, _token))
        ...
        protected override async Task SendPreRequestsAsync()
        {
            await this.E2ETester
                ...
                .Tap(async (result) => this._token = await result.GetResponseContentAsync<JsonWebToken>())
                .Back()
                .SendSynchronously(CreatePublisher())
