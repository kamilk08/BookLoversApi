...
namespace BookLovers.Publication.Tests.EndToEndTests.EditAuthorTests
{
    public class EditAuthorTest : EndToEndTest
    {
        ...
        [Test]
        public async Task EditAuthor_WhenCalled_ShouldReturnResponseWithOkStatusCode()
        {
            await this.E2ETester
                .ArePrecedingRequestsSuccessful((flag) => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(() => new EditAuthorHttpRequest(this.CreateCommandData(), _token))
        ...
        private EditAuthorWriteModel CreateCommandData()
        {
            var authorName = Fixture.Create<string>();

            var dto = Fixture.Build<AuthorWriteModel>()
                .WithBooks(new List<Guid>() { _bookGuid })
                .WithBasics(Sex.Male, authorName, "DUPA")
                ...
                .With(p => p.AuthorGuid, _authorGuid)
                .Create<AuthorWriteModel>();

            var editAuthorDto = Fixture.Build<EditAuthorWriteModel>()
                .With(p => p.AuthorWriteModel, dto)
                .With(p => p.PictureWriteModel, new AuthorPictureWriteModel
                {
                    FileName = string.Empty,
                    AuthorImage = string.Empty
                })
                .Create<EditAuthorWriteModel>();

            return editAuthorDto;
        }
