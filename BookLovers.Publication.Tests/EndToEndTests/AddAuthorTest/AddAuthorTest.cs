...
{
    public class AddAuthorTest : EndToEndTest
    {
        ...
        [Test]
        public async Task CreateAuthor_WhenCalled_ShouldReturnResponseWithCreatedStatusCode()
        {
            var readersModule = new ReadersModule();
            var ratingsModule = new RatingsModule();
            ...
            await this.E2ETester
                ...
                .SendAsyncAndMapToResult(() => CreateAuthor())
                .Tap((result) => result.IsCreated().Should().BeTrue())
                .Back()
                .AddTask(async () => await poller.Check(new AddAuthorReadersModuleProbe(readersModule, _authorGuid)))
                .AddTask(async () => await poller.Check(new AddAuthorRatingsModuleProbe(ratingsModule, _authorGuid)))
                .DoAsync();
        }
