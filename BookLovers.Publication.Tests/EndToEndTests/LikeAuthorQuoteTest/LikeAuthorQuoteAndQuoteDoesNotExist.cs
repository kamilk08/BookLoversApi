...
namespace BookLovers.Publication.Tests.EndToEndTests.LikeAuthorQuoteTest
{
    public class LikeAuthorQuoteAndQuoteDoesNotExist : EndToEndTest
    {
        ...
        [Test]
        public async Task LikeQuote_WhenCalledAndQuoteDoesNotExist_ShouldReturnResponseWithConflictStatusCode()
        {
            await this.E2ETester
                .ArePrecedingRequestsSuccessful((flag) => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(() => new LikeAuthorQuoteHttpRequest(Fixture.Create<Guid>(), _token))
