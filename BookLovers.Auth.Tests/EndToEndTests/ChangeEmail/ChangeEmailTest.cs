...
{
    public class ChangeEmailTest : EndToEndTest
    {
        ...
        [Test]
        public async Task ChangeEmail_WhenCalled_ShouldChangeUsersEmailAndReturnOkStatusCode()
        {
            await this.E2ETester
                .ArePrecedingRequestsSuccessful(flag => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(new ChangeEmailHttpRequest(
        ...
        protected override void InitializeModules(
            ...
            IAppManager manager)
        {
            AuthModuleStartup.Initialize(contextAccessor, manager, Log.Logger, PersistenceSettings.Default());
        }
        ...
        protected override async Task SendPreRequestsAsync()
        {
            await this.E2ETester
                ...
                .Tap(async (response) => this._registrationToken = await response.GetResponseContentAsync<string>())
                .Back()
