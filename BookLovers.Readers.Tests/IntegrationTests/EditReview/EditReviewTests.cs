...
{
    public class EditReviewTests : IntegrationTest<ReadersModule, EditReviewCommand>
    {
        ...
        [Test]
        public async Task EditReview_WhenCalledAndCommandIsNotValid_ShouldReturnFailureResult()
        {
            Command = null;
        ...
        [Test]
        public async Task EditReview_WhenCalledAndCommandDataIsNotValid_ShouldReturnFailureResult()
        {
            _reviewWriteModel = Fixture.Build<ReviewWriteModel>()
                .With(p => p.BookGuid, Guid.Empty)
                .With(p => p.ReviewGuid, Guid.Empty)
                .With(p => p.ReviewDetails)
                .Create();

            var validationResult = await this.Module.SendCommandAsync(new EditReviewCommand(_reviewWriteModel));
            validationResult.HasErrors.Should().BeTrue();
            validationResult.Errors.Should().ContainSingle(p => p.ErrorProperty == "BookGuid");
            validationResult.Errors.Should().ContainSingle(p => p.ErrorProperty == "ReviewGuid");
        }

        [Test]
        public void
            EditReview_WhenCalledAndSomeoneButNotOwnerTriesToEditReview_ShouldThrowBusinessRuleNotMeetException()
        {
            _httpAccessorMock.Setup(s => s.UserGuid).Returns(Fixture.Create<Guid>());
