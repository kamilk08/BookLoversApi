...
namespace BookLovers.Readers.Tests.EndToEndTests.UnLikeReviewTests
{
    public class UnLikeReviewAndReviewDoesNotExist : EndToEndTest
    {
        ...
        [Test]
        public async Task UnLikeReview_WhenCalledAndReviewDoesNotExist_ShouldReturnResponseWithConflictStatusCode()
        {
            await this.E2ETester
                .ArePrecedingRequestsSuccessful((flag) => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(() => new UnLikeReviewHttpRequest(Fixture.Create<Guid>(), _token))
        ...
        protected override async Task SendPreRequestsAsync()
        {
            await E2ETester
                ...
                .SendAsync(() => new LikeReviewRequest(_reviewGuid, _token));
        }
