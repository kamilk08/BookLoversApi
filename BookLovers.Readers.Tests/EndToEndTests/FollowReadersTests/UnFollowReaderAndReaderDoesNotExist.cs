...
{
    public class UnFollowReaderAndReaderDoesNotExist : EndToEndTest
    {
        ...
        [Test]
        public async Task UnFollowReader_WhenCalledAndReaderDoesNotExist_ShouldReturnResponseWithConflictStatus()
        {
            await this.E2ETester
                .ArePrecedingRequestsSuccessful((flag) => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(() => new UnFollowReaderHttpRequest(Fixture.Create<Guid>(), _token))
        ...
        protected override async Task SendPreRequestsAsync()
        {
            await this.E2ETester
                ...
                .SendPendingRequestsAsync()
                .SendSynchronouslyAndMapToResult(LoginFollower)
                .Tap(async (response) =>
                {
                    _token = await response.GetResponseContentAsync<JsonWebToken>();
                })
                .Back()
