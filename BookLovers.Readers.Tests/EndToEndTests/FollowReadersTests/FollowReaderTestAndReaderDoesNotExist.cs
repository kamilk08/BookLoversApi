...
{
    public class FollowReaderTestAndReaderDoesNotExist : EndToEndTest
    {
        ...
        [Test]
        public async Task FollowReader_WhenCalledAndReaderDoesNotExist_ShouldReturnResponseWithConflictStatusCode()
        {
            await this.E2ETester
                .ArePrecedingRequestsSuccessful((flag) => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(() => new FollowReaderHttpRequest(Fixture.Create<Guid>(), _token))
