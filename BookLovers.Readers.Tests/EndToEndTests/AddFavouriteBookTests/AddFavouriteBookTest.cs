...
{
    public class AddFavouriteBookTest : EndToEndTest
    {
        ...
        [Test]
        public async Task AddFavouriteBook_WhenCalled_ShouldReturnResponseWithOkStatusCode()
        {
            await this.E2ETester
                .ArePrecedingRequestsSuccessful((flag) => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(() =>
                    new AddFavouriteBookHttpRequest(_bookGuid, _profileGuid, _token))
        ...
        {
            return new CreatePublisherHttpRequest(_token).WithPublisherGuid(_publisherGuid);
        }
