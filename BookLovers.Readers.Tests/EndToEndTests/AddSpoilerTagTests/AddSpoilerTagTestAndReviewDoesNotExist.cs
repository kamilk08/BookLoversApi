...
namespace BookLovers.Readers.Tests.EndToEndTests.AddSpoilerTagTests
{
    public class AddSpoilerTagTestAndReviewDoesNotExist : EndToEndTest
    {
        ...
        [Test]
        public async Task AddSpoilerTag_WhenCalledAndReviewDoesNotExist_ShouldReturnResponseWithConflictStatusCode()
        {
            await E2ETester
                .ArePrecedingRequestsSuccessful((flag) => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(() => new AddSpoilerTagHttpRequest(_reviewGuid, _token))
