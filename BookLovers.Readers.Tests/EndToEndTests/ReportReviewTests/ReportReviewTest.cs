...
{
    public class ReportReviewTest : EndToEndTest
    {
        ...
        [Test]
        public async Task ReportReview_WhenCalled_ShouldReturnResponseWithOkStatusCode()
        {
            var reportReviewTest = this;
            IModule<LibrarianModule> module = new LibrarianModule();
            var poller = new Poller(TimeChecker.WithSeconds(30));
            var simpleE2Etester = await reportReviewTest.E2ETester
                .ArePrecedingRequestsSuccessful(flag => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(() =>
                    (ISimpleHttpRequest) new ReportReviewHttpRequest(_reviewGuid, ReportReason.Spam, _token))
                .Tap(result => result.IsOk().Should().BeTrue()).Back().AddTaskAsync(async () =>
                    await poller.Check(new ReportReviewLibrarianModuleProbe(module))).DoAsync();
        }
