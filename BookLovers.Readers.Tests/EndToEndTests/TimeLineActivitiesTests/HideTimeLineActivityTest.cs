...
{
    public class HideTimeLineActivityTest : EndToEndTest
    {
        ...
        [Test]
        public async Task HideTimeLineActivity_WhenCalled_ShouldReturnResponseWithOkStatus()
        {
            await this.E2ETester
                .ArePrecedingRequestsSuccessful((flag) => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(() => new HideActivityHttpRequest(_occuredAt, _timeLineObjectGuid, _token))
        ...
        protected override void RegisterServices(IKernel kernel)
        {
            ...
            _appManagerMock.Setup(s => s.GetConfigValue(BookcaseContext.ConnectionStringKey))
                .Returns(bookcaseConnectionString);

            _appManagerMock.Setup(s => s.GetConfigValue(BookcaseStoreContext.ConnectionStringKey))
