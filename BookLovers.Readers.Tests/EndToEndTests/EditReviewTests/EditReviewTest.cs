...
namespace BookLovers.Readers.Tests.EndToEndTests.EditReviewTests
{
    public class EditReviewTest : EndToEndTest
    {
        ...
        [Test]
        public async Task EditReview_WhenCalled_ShouldReturnResponseWithOkStatusCode()
        {
            await E2ETester
                .ArePrecedingRequestsSuccessful((flag) => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(() => new EditProfileHttpRequest(Fixture.Create<Guid>(), _token))
        ...
        protected override async Task SendPreRequestsAsync()
        {
            await E2ETester
                ...
                .Act(() => _reviewGuid = Fixture.Create<Guid>())
        ...
        {
            return new CreateLibrarianHttpRequest(_token)
