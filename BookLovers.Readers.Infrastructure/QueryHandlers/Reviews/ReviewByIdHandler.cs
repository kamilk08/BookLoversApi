...
{
    internal class ReviewByIdHandler : IQueryHandler<ReviewByIdQuery, ReviewDto>
    {
        private readonly ReadersContext _context;
        private readonly IMapper _mapper;

        public ReviewByIdHandler(ReadersContext context, IMapper mapper)
        {
            this._context = context;
            this._mapper = mapper;
        }

        public async Task<ReviewDto> HandleAsync(ReviewByIdQuery query)
        {
            var review = await this._context.Reviews
                ...
                .AsNoTracking()
                .ActiveRecords()
                .SingleOrDefaultAsync(p => p.Id == query.ReviewId);
