...
{
    internal class ReaderTimeLineHandler : IQueryHandler<ReaderTimeLineQuery, TimeLineDto>
    {
        private readonly ReadersContext _context;
        private readonly IMapper _mapper;

        public ReaderTimeLineHandler(ReadersContext context, IMapper mapper)
        {
            this._context = context;
            this._mapper = mapper;
        }

        public async Task<TimeLineDto> HandleAsync(ReaderTimeLineQuery query)
        {
            var timeline = await this._context.TimeLines
                .AsNoTracking()
                .ActiveRecords()
