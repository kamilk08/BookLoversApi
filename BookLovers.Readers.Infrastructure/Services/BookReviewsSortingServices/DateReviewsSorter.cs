...
{
    internal class DateReviewsSorter : IReviewsSorter
    {
        private readonly ReadersContext _context;
        private readonly IMapper _mapper;

        public ReviewsSortingType SortingType => ReviewsSortingType.ByDate;

        public DateReviewsSorter(ReadersContext context, IMapper mapper)
        {
            ...
            PaginatedBookReviewsQuery query)
        {
            var baseQuery = this._context.Reviews
                ...
                .Where(p => p.Book.BookId == query.BookId).WithContent();
            ...
            if (!query.Descending)
                orderedQuery = baseQuery.OrderBy(p => p.CreateDate);
            else
                orderedQuery = baseQuery.OrderByDescending(p => p.CreateDate);

            var paginatedReviewsQuery = orderedQuery
            ...
            var paginatedResult = new PaginatedResult<ReviewDto>(
                mappedResults, query.Page, query.Count, totalReviews);
