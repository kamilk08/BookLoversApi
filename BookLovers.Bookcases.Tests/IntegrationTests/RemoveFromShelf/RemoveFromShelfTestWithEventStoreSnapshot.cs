...
{
    public class RemoveFromShelfTestWithEventStoreSnapshot :
        IntegrationTest<BookcaseModule, RemoveFromShelfCommand>
    {
        ...
        public async Task RemoveFromShelf_WhenCalled_ShouldRemoveBookFromShelf()
        {
            ...
            queryResult.Value.Shelves.SelectMany(sm => sm.Publications).Count().Should().Be(0);
        }
        ...
        protected override async Task PrepareData()
        {
            ...
            await Enumerable.Range(0, 5).ForEach(async i =>
            {
                var book = new BookcaseBook(
                    Fixture.Create<Guid>(),
                    Fixture.Create<Guid>(), Fixture.Create<int>());
            ...
            await Enumerable.Range(0, 5).ForEach(async i =>
            {
                await Module.SendCommandAsync(new AddToBookcaseCommand(new AddBookToBookcaseWriteModel()
                {
                    ...
                    ShelfGuid = bookcase.Shelves.First().Guid
                }));
            });

            await Enumerable.Range(0, 4).ForEach(async i =>
            {
                await Module.SendCommandAsync(new RemoveFromShelfCommand(new RemoveFromShelfWriteModel()
                {
                    BookcaseGuid = _bookcaseGuid,
                    BookGuid = _books[i].BookGuid,
                    ShelfGuid = bookcase.Shelves.First().Guid
