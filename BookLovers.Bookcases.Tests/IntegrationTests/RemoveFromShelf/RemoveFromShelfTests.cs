...
namespace BookLovers.Bookcases.Tests.IntegrationTests.RemoveFromShelf
{
    [TestFixture]
    public class RemoveFromShelfTests : IntegrationTest<BookcaseModule, RemoveFromShelfCommand>
    {
        ...
        [Test]
        public async Task RemoveFromShelf_WhenCalled_ShouldRemoveBookFromShelf()
        {
            var queryResult = await Module.ExecuteQueryAsync<ShelfByNameQuery, ShelfDto>(
                new ShelfByNameQuery(_bookcaseId, _shelfName));

            queryResult.Value.PublicationsCount.Should().Be(1);

            var validationResult = await Module.SendCommandAsync(Command);
            validationResult.HasErrors.Should().BeFalse();

            queryResult = await Module.ExecuteQueryAsync<ShelfByNameQuery, ShelfDto>(
                new ShelfByNameQuery(_bookcaseId, _shelfName));

            queryResult.Value.PublicationsCount.Should().Be(0);
        }

        [Test]
        public async Task RemoveFromShelf_WhenCalledAndCommandIsNull_ShouldReturnValidationResultWithErrors()
        {
            Command = null;
        ...
        [Test]
        public async Task RemoveFromShelf_WhenCalledAndCommandHasInvalidData_ShouldReturnValidationResultWithErrors()
        {
            Command = new RemoveFromShelfCommand(new RemoveFromShelfWriteModel()
            {
