...
{
    public class ChangeShelfToWantToReadShelfTest : EndToEndTest
    {
        ...
        private ShelfDto _shelfDto;
        private ReaderDto _readerDto;
        private BookcaseDto _bookcaseDto;

        [Test]
        public async Task ChangeShelf_WhenCalled_ShouldReturnResponseWithOkStatusCode()
        {
            await E2ETester
                .ArePrecedingRequestsSuccessful(flag => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(new ChangeShelfHttpRequest(_bookcaseGuid, _shelfGuid, _shelfDto.Guid,
                    _bookGuid,
                    _token))
        ...
        protected override async Task SendPreRequestsAsync()
        {
            await E2ETester
                ...
                .Act(() =>
                {
                    _shelfDto = _bookcaseDto.Shelves.Single(p => p.ShelfCategory == ShelfCategory.WantToRead.Value);
                });
        }
