...
namespace BookLovers.Bookcases.Tests.EndToEndTests.ChangeShelfTests
{
    public class ChangeShelfAndUserDoesNotHaveSelectedShelf : EndToEndTest
    {
        ...
        [Test]
        public async Task
            ChangeShelf_WhenCalledAndUserDoesNotHaveSelectedShelf_ShouldReturnResponseWithConflictStatusCode()
        {
            await E2ETester
                .ArePrecedingRequestsSuccessful((flag) => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(() =>
                    new ChangeShelfHttpRequest(_bookcaseGuid, _shelfGuid, Fixture.Create<Guid>(), _bookGuid, _token))
        ...
        protected override async Task SendPreRequestsAsync()
        {
            await E2ETester
                ...
                .AddRequestAsync(() => CreateBook())
                .AddRequestAsync(
                    () => new AddShelfHttpRequest(_bookcaseGuid, _token).WithShelfGuid(_shelfGuid))
                .AddRequestAsync(() => new AddToShelfHttpRequest(_bookcaseGuid, _shelfGuid, _bookGuid, _token))
                .SendPendingRequestsAsync();
        }
