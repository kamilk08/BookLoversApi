...
{
    public class ChangeShelfTwoWantToReadAndUserIsNotAnOwnerOfTheBookcaseTest : EndToEndTest
    {
        ...
        [Test]
        public async Task
            ChangeShelf_WhenCalledAndUserIsNotAnOwnerOfTheBookcase_ShouldReturnResponseWithForbiddenStatusCode()
        {
            await E2ETester
                .ArePrecedingRequestsSuccessful((flag) => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(() =>
                    new ChangeShelfHttpRequest(_bookcaseGuid, _shelfGuid, Guid.NewGuid(), _bookGuid, _token))
