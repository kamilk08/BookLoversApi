...
namespace BookLovers.Bookcases.Tests.EndToEndTests.ChangeShelfNameTests
{
    public class ChangeShelfNameAndUserIsNotAnOwnerOfTheBookcaseTest : EndToEndTest
    {
        ...
        private string _newShelfName = "CHANGE SHELF NAME";

        [Test]
        public async Task
            ChangeShelfName_WhenCalledAndUserIsNotAnOwnerOfTheBookcase_ShouldReturnResponseWithUnAuthorizedStatusCode()
        {
            await E2ETester
                .SendAsyncAndMapToResult(() =>
                    new ChangeShelfNameRequest(_bookcaseGuid, _shelfGuid, _newShelfName, _token))
        ...
        protected override async Task SendPreRequestsAsync()
        {
            await E2ETester
                ...
                .AddRequestAsync(() =>
                    new CompleteRegistrationHttpRequest(E2EConstants.DefaultUserEmail, _secondRegistrationToken))
                .SendPendingRequestsAsync()
                .SendSynchronouslyAndMapToResult(() => LoginUser())
                .Tap(async (result) => _token = await result.GetResponseContentAsync<JsonWebToken>())
                .Back()
                .SendAsync(
                    () => new AddShelfHttpRequest(_bookcaseGuid, _token).WithShelfGuid(_shelfGuid))
