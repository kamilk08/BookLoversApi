...
{
    public class AddShelfTest : EndToEndTest
    {
        ...
        [Test]
        public async Task AddShelf_WhenCalled_ShouldReturnResponseWithCreatedStatusCode()
        {
            IModule<ReadersModule> module = new ReadersModule();
            var poller = new Poller(TimeChecker.WithSeconds(30));

            await E2ETester
                .SendAsyncAndMapToResult(() => new AddShelfHttpRequest(_bookcaseGuid, _token))
                .Tap((result) => result.IsCreated().Should().BeTrue())
                .Back()
                .AddTask(async () => await poller.Check(new AddShelfReadersModuleProbe(module)))
                .DoAsync()
