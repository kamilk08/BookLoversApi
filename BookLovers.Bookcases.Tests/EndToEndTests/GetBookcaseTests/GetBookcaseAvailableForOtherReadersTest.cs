...
{
    public class GetBookcaseAvailableForOtherReadersTest : EndToEndTest
    {
        ...
        [Test]
        public async Task GetBookcaseAvailableForOtherReaders_WhenCalled_ShouldReturnResponseWithOkStatusCode()
        {
            BookcaseDto bookcase = null;
            ...
            await E2ETester
                ...
                .Tap(async (result) => bookcase = await result.GetResponseContentAsync<BookcaseDto>());

            bookcase.Should().NotBeNull();
        }
        ...
        protected override void RegisterServices(IKernel kernel)
        {
            ...
            _managerMock.Setup(s => s.GetConfigValue(JwtSettings.Issuer)).Returns("http://localhost:64892/");
            _managerMock.Setup(s => s.GetConfigValue(JwtSettings.JsonWebTokenKey)).Returns("mySuperSecretKey");
            ...
            _managerMock.Setup(s => s.GetConfigValue(BookcaseContext.ConnectionStringKey))
                .Returns(bookcaseConnectionString);

            _managerMock.Setup(s => s.GetConfigValue(BookcaseStoreContext.ConnectionStringKey))
                .Returns(bookcaseStoreConnectionString);

            _managerMock.Setup(s => s.GetConfigValue(PublicationsContext.ConnectionStringKey))
                .Returns(publicationsConnectionString);

            _managerMock.Setup(s => s.GetConfigValue(PublicationsStoreContext.ConnectionStringKey))
            ...
            _managerMock.Setup(s => s.GetConfigValue(RatingsContext.ConnectionStringKey))
                .Returns(ratingsConnectionString);

            _managerMock.Setup(s => s.GetConfigValue(ReadersContext.ConnectionStringKey))
                .Returns(readersConnectionString);

            _managerMock.Setup(s => s.GetConfigValue(ReadersStoreContext.ConnectionStringKey))
                .Returns(readersStoreConnectionString);

            kernel.Bind<IHttpContextAccessor>().ToConstant(httpContextAccessor);
            kernel.Bind<IAppManager>().ToConstant(_managerMock.Object);
        ...
        protected override async Task SendPreRequestsAsync()
        {
            await E2ETester
                ...
                .Back()
                .SendAsync(() => new ChangeBookcaseSettingsHttpRequest(_bookcaseGuid, _token)
                    .AddPrivacyOption(BookcaseOptionType.Privacy, PrivacyOption.OtherReaders))
