...
{
    public class RemoveFromShelfAndShelfDoesNotExist : EndToEndTest
    {
        ...
        [Test]
        public async Task RemoveFromShelf_WhenCalled_ShouldReturnResponseWithConflictStatusCode()
        {
            await E2ETester
                .ArePrecedingRequestsSuccessful((flag) => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(() =>
                    new RemoveFromShelfRequest(_bookcaseGuid, Fixture.Create<Guid>(), _bookGuid, _token))
        ...
        protected override void RegisterServices(IKernel kernel)
        {
            ...
            appManagerMock.Setup(s => s.GetConfigValue(AuthContext.ConnectionStringKey))
                .Returns(@"Server=localhost\SQLEXPRESS;Database=AuthorizationContext;Trusted_Connection=True;");

            appManagerMock.Setup(s => s.GetConfigValue(BookcaseContext.ConnectionStringKey))
                .Returns(@"Server=localhost\SQLEXPRESS;Database=BookcaseContext;Trusted_Connection=True;");

            appManagerMock.Setup(s => s.GetConfigValue(BookcaseStoreContext.ConnectionStringKey))
                .Returns(@"Server=localhost\SQLEXPRESS;Database=BookcaseStoreContext;Trusted_Connection=True;");

            appManagerMock.Setup(s => s.GetConfigValue(PublicationsContext.ConnectionStringKey))
                .Returns(@"Server=localhost\SQLEXPRESS;Database=PublicationsContext;Trusted_Connection=True;");
            ...
            appManagerMock.Setup(s => s.GetConfigValue(LibrariansContext.ConnectionStringKey))
                .Returns(@"Server=localhost\SQLEXPRESS;Database=LibrariansContext;Trusted_Connection=True;");

            appManagerMock.Setup(s => s.GetConfigValue(RatingsContext.ConnectionStringKey))
                .Returns(@"Server=localhost\SQLEXPRESS;Database=RatingsContext;Trusted_Connection=True;");

            appManagerMock.Setup(s => s.GetConfigValue(ReadersContext.ConnectionStringKey))
                .Returns(@"Server=localhost\SQLEXPRESS;Database=ReadersContext;Trusted_Connection=True;");

            appManagerMock.Setup(s => s.GetConfigValue(ReadersStoreContext.ConnectionStringKey))
                .Returns(@"Server=localhost\SQLEXPRESS;Database=ReadersStoreContext;Trusted_Connection=True;");
