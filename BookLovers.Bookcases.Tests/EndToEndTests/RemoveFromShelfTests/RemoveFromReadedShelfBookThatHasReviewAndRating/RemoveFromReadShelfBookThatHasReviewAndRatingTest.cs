...
namespace BookLovers.Bookcases.Tests.EndToEndTests.RemoveFromShelfTests.RemoveFromReadedShelfBookThatHasReviewAndRating
{
    public class RemoveFromReadShelfBookThatHasReviewAndRatingTest : EndToEndTest
    {
        private string _registrationSummaryToken;
        ...
        public async Task RemoveFromShelf_WhenCalled_ShouldReturnResponseWithOkStatusCode()
        {
            ...
            await E2ETester.SendAsyncAndMapToResult(() =>
                ...
                .AddTask(
                    async () => await poller.Check(new RemoveFromReadShelfLibrariansModuleProbe(librarianModule)))
                .DoAsync();
        }
        ...
        protected override async Task SendPreRequestsAsync()
        {
            await E2ETester
                ...
                .Tap(async (response) => _registrationSummaryToken = await response.GetResponseContentAsync<string>())
                .Back()
                .SendAsync(() =>
                    new CompleteRegistrationHttpRequest(E2EConstants.DefaultUserEmail, _registrationSummaryToken))
                ...
                .SendAsync(() =>
                    new AddToShelfHttpRequest(
                        ...
                        _token))
                .SendSynchronously(new AddReviewHttpRequest(Guid.NewGuid(), _bookGuid, _token))
                .SendSynchronously(new AddRatingHttpRequest(Star.Four, _bookGuid, _token));
        }
        ...
        private async Task<ShelfReadModel> GetShelf()
        {
            return await Kernel.Get<BookcaseContext>()
                .Bookcases
                .Where(p => p.ReaderGuid == _userGuid)
                .SelectMany(sm => sm.Shelves)
                .SingleOrDefaultAsync(p => p.ShelfCategory == ShelfCategory.Read.Value);
        }
