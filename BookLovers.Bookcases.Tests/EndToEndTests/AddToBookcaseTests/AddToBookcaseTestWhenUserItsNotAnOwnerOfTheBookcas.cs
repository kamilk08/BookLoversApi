...
{
    public class AddToBookcaseTestWhenUserItsNotAnOwnerOfTheBookcase : EndToEndTest
    {
        ...
        [Test]
        public async Task
            AddToBookcase_WhenCalledAndUserIsNotAnOwnerOfTheBookcase_ShouldReturnResponseWithForbiddenStatusCode()
        {
        ...
        protected override async Task SendPreRequestsAsync()
        {
            await E2ETester
                ...
                {
                    var httpContextAccessor = new FakeHttpContextAccessor(_secondUserGuid, true, false, false);
                    Kernel.Rebind<IHttpContextAccessor>().ToConstant(httpContextAccessor);
                }));
        }
