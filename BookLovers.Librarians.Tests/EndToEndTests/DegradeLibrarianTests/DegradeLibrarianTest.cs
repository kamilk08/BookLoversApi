...
namespace BookLovers.Librarians.Tests.EndToEndTests.DegradeLibrarianTests
{
    public class DegradeLibrarianTest : EndToEndTest
    {
        ...
        [Test]
        public async Task DegradeLibrarian_WhenCalled_ShouldReturnResponseWithOkStatusCode()
        {
            IModule<AuthModule> authModule = Kernel.Get<IModule<AuthModule>>();
            var poller = new Poller(TimeChecker.WithSeconds(15));

            await this.E2ETester
                .ArePrecedingRequestsSuccessful((flag) => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(() => new DegradeLibrarianHttpRequest(_userGuid, _token))
                .Tap((result) => result.IsOk().Should().BeTrue())
                .Back()
                .AddTask(async () => await poller.Check(new DegradeLibrarianAuthModuleProbe(authModule, _userGuid)))
                .DoAsync();
        }
