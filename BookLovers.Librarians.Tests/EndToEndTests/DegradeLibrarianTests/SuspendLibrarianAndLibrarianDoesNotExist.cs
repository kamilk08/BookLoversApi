...
{
    public class SuspendLibrarianAndLibrarianDoesNotExist : EndToEndTest
    {
        ...
        [Test]
        public async Task
            DegradeLibrarian_WhenCalledAndLibrarianDoesNotExist_ShouldReturnResponseWithConflictStatusCode()
        {
            await this.E2ETester
                .ArePrecedingRequestsSuccessful((flag) => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(() => new DegradeLibrarianHttpRequest(Fixture.Create<Guid>(), _token))
