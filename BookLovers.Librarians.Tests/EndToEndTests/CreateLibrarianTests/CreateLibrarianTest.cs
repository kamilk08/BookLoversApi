...
{
    public class CreateLibrarianTest : EndToEndTest
    {
        ...
        [Test]
        public async Task CreateLibrarian_WhenCalled_ShouldReturnResponseWithCreatedStatusCode()
        {
            ...
            await this.E2ETester
                ...
                .SendAsyncAndMapToResult(() => new CreateLibrarianHttpRequest(_token).WithReaderGuid(_userGuid))
                .Tap((result) => result.IsCreated().Should().BeTrue())
                .Back()
                .AddTask(async () =>
                    await poller.Check(new CreateLibrarianAuthModuleProbe(authModule, _userGuid)))
                .DoAsync();
        }
