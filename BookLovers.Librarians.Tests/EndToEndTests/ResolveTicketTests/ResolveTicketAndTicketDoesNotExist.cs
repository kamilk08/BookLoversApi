...
{
    public class ResolveTicketAndTicketDoesNotExist : EndToEndTest
    {
        ...
        private FakeHttpContextAccessor _firstUserLoginMock;
        private FakeHttpContextAccessor _secondUserHttpContextAccessor;

        [Test]
        public async Task ResolveTicket_WhenCalledAndTicketDoesNotExist_ShouldReturnResponseWithConflictStatusCode()
        {
            await this.E2ETester
                .ArePrecedingRequestsSuccessful((flag) => flag.Should().BeTrue())
                .SendAsyncAndMapToResult(() =>
                    new ResolveTicketHttpRequest(Fixture.Create<Guid>(), _librarianGuid, Decision.Approve,
                        TicketConcern.Book,
                        _token))
        ...
        protected override void RegisterServices(IKernel kernel)
        {
            ...
            _firstUserLoginMock = new FakeHttpContextAccessor(_userGuid, true);
            ...
            kernel.Bind<IHttpContextAccessor>().ToConstant(_firstUserLoginMock);
        ...
        {
            var bookWriteModel = Fixture.Build<BookWriteModel>().With(p => p.BookGuid, _bookGuid)
                .With(p => p.Description)
                .WithBookBasics(Category.Fiction, SubCategory.FictionSubCategory.Action, "9788375155280",
                    Fixture.Create<string>(), _publisherGuid)
                .WithDetails(Fixture.Create<int>(), Language.English).WithCover(CoverType.PaperBack, false)
                .WithSeries(_seriesGuid, 1)
                .With(p => p.AddedBy, _secondUserGuid)
            ...
            var writeModel = Fixture.Build<CreateBookWriteModel>()
                .With(p => p.BookWriteModel, bookWriteModel).With(p => p.PictureWriteModel, new BookPictureWriteModel()
                {
                    Cover = string.Empty,
                    FileName = string.Empty
                }).Create();

            return JsonConvert.SerializeObject(writeModel);
        }
