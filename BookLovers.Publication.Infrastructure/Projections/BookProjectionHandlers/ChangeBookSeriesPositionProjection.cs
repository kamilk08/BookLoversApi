...
{
    internal class ChangeBookSeriesPositionProjection :
        IProjectionHandler<BookSeriesPositionChanged>,
        IProjectionHandler
    {
        private readonly PublicationsContext _context;

        public ChangeBookSeriesPositionProjection(PublicationsContext context)
        {
            this._context = context;
        }

        public void Handle(BookSeriesPositionChanged @event)
        {
            _context.Books.Where(p => p.Guid == @event.AggregateGuid)
                .Update(p => new BookReadModel
                {
                    PositionInSeries = @event.Position
                });
        }
    }
}